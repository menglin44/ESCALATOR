...
for ii,line in enumerate(infile):
    if pastheader:
        if ii == nheader+1:
            outfile = filename.replace('.gz', '') .replace('.txt', '_' + build + '_reformated.txt')
            outfile = open(outfile, 'w')
            if flag_dbsnp:
                rs2otherfields={} # prepare a dict of input file with rsID:effectA_refA_weight
        ...
        if version != '2':
            print 'Formatting the input weight file: assuming the format version is in PGS catalog v1.0'
            oa_field = 'reference_allele'
        else:
            print 'Formatting the input weight file: assuming the format version is in PGS catalog v2.0'
            oa_field = 'other_allele'
        minimum = ['rsID', 'effect_allele', oa_field, 'effect_weight']
        required = ['chr_name', 'chr_position', 'effect_allele', oa_field, 'effect_weight']
        stringent = ['rsID', 'chr_name', 'chr_position', 'effect_allele', oa_field, 'effect_weight'] # some file might not have the field 'rsID'
        if set(stringent).issubset(set(line)): # ideally, all wanted fields present
            ...
            assert set(minimum).issubset(set(line)), "Error: Requiring fields of effect_allele, reference_allele/other_allele, effect_weight, and either chr + bp or rsID."
            print "The input file has snpID, but no chr and bp columns (both). Will update position info with dbSNP."
            build = "hg38"
...
if flag_dbsnp:
    ...
    for snp in original_snps:
        outfile.write('%s\t%s\n' % (snp, '\t'.join(snp2pos.get(snp, 'NA_NA').split('_') + rs2otherfields.get(snp, 'NA_NA_NA').split('_'))))
